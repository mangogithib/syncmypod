...
class TestQuality:
    """The settings the page can change.

    Stored per computer rather than on the server, because it is a decision
    about this machine and this iPod - what will fit and what this connection
    will download - not about the library.
    """

    @pytest.fixture
    def paired(self, tmp_path, monkeypatch):
    ...
    def test_state_carries_the_settings_and_the_choices(self, server, paired):
        _status, body = call(server, "/api/state", token=server.session.token)
        assert body["quality"]["name"] == "balanced"
        assert "high" in body["qualityChoices"]["presets"]
        assert 192 in body["qualityChoices"]["bitrates"]

    def test_a_preset_can_be_chosen(self, server, paired):
        _status, body = call(
            server, "/api/quality", token=server.session.token,
            method="POST", body={"preset": "compact"},
        )
        assert body["saved"]
        assert body["quality"]["name"] == "compact"
        assert body["quality"]["maxBitrateKbps"] == 128

    def test_one_field_can_be_changed_without_touching_the_rest(self, server, paired):
        _status, body = call(
            server, "/api/quality", token=server.session.token,
            method="POST", body={"minSourceKbps": 128},
        )
        assert body["quality"]["minSourceKbps"] == 128
        assert body["quality"]["maxBitrateKbps"] == 192
        assert body["quality"]["name"] == "custom"

    def test_it_survives_for_the_next_run(self, server, paired):
        call(
            server, "/api/quality", token=server.session.token,
            method="POST", body={"preset": "high"},
        )
        from syncmypod_local import config

        assert config.load().quality.name == "high"

    def test_nonsense_is_ignored_rather_than_stored(self, server, paired):
        """The page is the only caller, but it is still a public endpoint."""
        _status, body = call(
            server, "/api/quality", token=server.session.token,
            method="POST",
            body={"codec": "flac", "maxBitrateKbps": "loud", "preferNoReencode": "yes"},
        )
        assert body["quality"]["codec"] == "aac"
        assert body["quality"]["maxBitrateKbps"] == 192

    def test_out_of_range_numbers_are_clamped(self, server, paired):
        _status, body = call(
            server, "/api/quality", token=server.session.token,
            method="POST", body={"maxBitrateKbps": 100000},
        )
        assert body["quality"]["maxBitrateKbps"] == 320

    def test_it_needs_the_token(self, server, paired):
        with pytest.raises(HTTPError) as raised:
            call(server, "/api/quality", method="POST", body={"preset": "high"})
        assert raised.value.code == 401
